...
import com.cmsystem.wms.service.LoginRecordService;
...
public class LogRegController {
    ...
    Result login(@Validated({UserInfo.Login.class}) @RequestBody UserInfo userForm, BindingResult bindingResult) {
        if (bindingResult.hasErrors()) {
            return Result.errorByBindingResult(bindingResult);
        }
        UserInfo user = userInfoMapper.findByUserName(userForm.getUserName());
        if (user == null ) {
        ...
        userInfoService.refreshUserToken(user);
